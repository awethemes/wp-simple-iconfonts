...
abstract class Extractor {
	...
	public function extract() {
		...
		if ( empty( $args['metadata_path'] ) || empty( $args['stylesheet_path'] ) || empty( $args['font_paths'] ) ) {
			return new WP_Error( 'invalid_directory_structure', esc_html__( 'Invalid directory structure.', 'wp_simple_iconfonts' ) );
		}

		if ( filesize( $args['metadata_path'] ) > 2097152 ) { // 2097152 = 2MB
			return new WP_Error( 'metadata_too_large', esc_html__( 'The metadata file too large, maximum is less than 2MB', 'wp_simple_iconfonts' ) );
		}

		// Make a new Result.
		$result = new Result( $this, $args );
		...
		if ( empty( $result->id ) || empty( $result->icons ) ) {
			return new WP_Error( 'invalid_iconpack', esc_html__( 'Invalid icon pack data.', 'wp_simple_iconfonts' ) );
		}
	 ...
	 *
	 * @param  Result $iconpack Extractor icon pack instance.
	 * @return void
	 */
	abstract protected function doing_extract( Result $iconpack );
}
